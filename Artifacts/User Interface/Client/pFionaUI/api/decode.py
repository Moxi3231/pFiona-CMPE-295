...
def get_last_fetched_file():
    last_fetched_file = os.getcwd()+"/Device/"+device_data[0]+"/last_fetched_file.json"
    with open(last_fetched_file, 'r') as f:
        last_file_data = json.load(f)
    return last_file_data

def update_last_getched_file(newData):
    last_fetched_file = os.getcwd()+"/Device/"+device_data[0]+"/last_fetched_file.json"
    file = open(last_fetched_file, 'w')
    json.dump(newData, file)
    file.close()
    return True

def update_all_data_json(newData):
    all_data_file = os.getcwd()+"/Device/"+device_data[0]+"/all_data_file.json"
    file = open(all_data_file, 'w')
    json.dump(newData, file)
    file.close()
    return True
...
def file_read(device_data):
    ...
    if data_files[0] == get_last_fetched_file()["lff"]:
        all_data_file = os.getcwd()+"/Device/"+device_data[0]+"/all_data_file.json"
        with open(all_data_file, 'r') as f:
            all_data_file = json.load(f)
        ...
        update_all_data_json(d)

        last_file_Data=get_last_fetched_file()
        last_file_Data["lff"]=data_files[0]
        update_last_getched_file(last_file_Data)
